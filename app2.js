...
window.onload=()=>{
  ...
  db.ref('portal_users').on('value', s => {
  if (s.exists()) {
    portalState.usersList = s.val();
    // On vérifie que les fonctions existent avant de les lancer pour éviter les crashs
    if (typeof renderFriendsList === 'function') renderFriendsList();
    if (typeof renderInboxAndHistory === 'function') renderInboxAndHistory();
    if (typeof updateProfileUI === 'function') updateProfileUI();
    if (typeof updateOnlineFriendsCount === 'function') updateOnlineFriendsCount();
  }
});
...
window.renderLobbiesUI = function() {
  let list = document.getElementById('lobbies-list');
  if (!list) return;

  // Sécurisation des objets pour éviter les crashs
  let portalLobbies = portalState.activeLobbies.portal || {};
  let quizLobbies = portalState.activeLobbies.quiz || {};
  let allLobbies = { ...portalLobbies, ...quizLobbies };
  let keys = Object.keys(allLobbies);

  // Mise à jour du badge rouge de notification
  let badge = document.getElementById('nav-lobbies-badge');
  if (badge) {
    badge.innerText = keys.length;
    badge.style.display = keys.length > 0 ? 'flex' : 'none';
